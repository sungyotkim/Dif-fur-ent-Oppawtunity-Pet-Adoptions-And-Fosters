...
function photoAdder(data) {
  let petsWithVideos = data.filter(
    (pet) => pet.videos.length > 0 && pet.photos.length > 1
  );
  let featuredPets = [];
  ...
  featuredPets.forEach((currentPet) => {
    ...
    img.src = `./dist/data/pets/${currentPet.name.toLowerCase()}/photos/${
      currentPet.photos[1]
    }`;
    img.setAttribute("width", "50px");
    img.setAttribute("class", "featured-img");
    const cardTitle = document.createElement("div");
    cardTitle.innerText = `${currentPet.name} the ${
      currentPet.characteristics[0]
    } ${currentPet.species === "Canine" ? "dog" : "cat"}`;
    cardTitle.setAttribute("class", "featured-card-title");
    const cardName = document.createElement("div");
    const cardDescription = document.createElement("div");
    cardName.innerText = `${currentPet.name} the ${
      currentPet.characteristics[0]
    } ${currentPet.species === "Canine" ? "dog" : "cat"}`;
...
function timer() {
  let countDownDate = new Date("June 10, 2023 00:00:00").getTime();
  ...
  let x = setInterval(function () {
    ...
    let goalTimer = (document.getElementById("goal-timer").innerHTML =
